...
void compute_eigensystem_of_original_mapping_no_rotation(cublasHandle_t handle, user_map_vector Axb, void *user_struct, int k, int m, int N, char which[2], real *residualAV_real, real *residualAV_imag, real complex *eigenvaluesH_kk, real complex *eigenvectorsH_kk, real complex *eigenvectorsH_kk_sorted, cublasComplex *eigenvectorsA_d, real *V_d, real *V1_d, real *Q_Schur_d){
    ...
    for(int i=0;i<k;i++){
        ...
        residualAV_real[i]=Arnoldi::vector_norm2_GPU(handle, N, &VRres_d[i*N]);
        residualAV_imag[i]=0.0;
    }
    //done

    real *H_Schur=new real[k*k];
    Arnoldi::to_host_from_device_real_cpy(H_Schur, Q_Schur_d, k, k, 1);
    Arnoldi::device_deallocate_all_real(2,  Vcolres_d, VRres_d);
...
real Implicit_restart_Arnoldi_GPU_data(cublasHandle_t handle, bool verbose, int N, user_map_vector Axb, void *user_struct, char which[2], int k, int m, complex real* eigenvaluesA, real tol, int max_iter, real *eigenvectors_real_d, real *eigenvectors_imag_d, int BLASThreads){
    //wrapper without external routine like matrix Exponent


    real ritz_norm=1.0;
    ritz_norm=Implicit_restart_Arnoldi_GPU_data_Matrix_Exponent(handle, verbose, N,  Axb, user_struct, Axb, user_struct, which, which, k, m, eigenvaluesA, tol, max_iter, false, eigenvectors_real_d, eigenvectors_imag_d, BLASThreads);
...
real Implicit_restart_Arnoldi_GPU_data_Matrix_Exponent(cublasHandle_t handle, bool verbose, int N,  user_map_vector Axb_exponent_invert, void *user_struct_exponent_invert, user_map_vector Axb, void *user_struct,  char which[2], char which_exponent[2], int k, int m, complex real* eigenvaluesA, real tol, int max_iter, bool is_rotated, real *eigenvectors_real_d, real *eigenvectors_imag_d, int BLASThreads){
    ...
    Arnoldi::device_allocate_all_real(N_rotational, m, 1, 2, &V_d, &V1_d);
    Arnoldi::device_allocate_all_real(N_rotational, 1, 1, 4, &vec_f_d, &vec_f1_d, &vec_w_d, &vec_v_d);
    ...
    while(((iterations++)<max_iter)&&(ritz_norm>tol)){
        ...
        real vec_f_norm=Arnoldi::vector_norm2_GPU(handle, N_rotational, vec_f_d);                  
        for(int i=0;i<k0;i++){
        ...
        Arnoldi::matrixMultVector_GPU(handle, N_rotational, V_d, m, hl, vec_q_d, ql, vec_f_d);
        ...
        Arnoldi::matrixMultMatrix_GPU(handle, N_rotational, m, m, V_d, 1.0, Q_d, 0.0, V1_d);   //OK
        
        //matrix_copy(N, m, V1, V);                                 //GG
        Arnoldi::vector_copy_GPU(handle, N_rotational*m, V1_d, V_d);
    ...
    //for rotated variant:
    // size(V)=n; <--- n is the double length due to rotation
    // V = V(:,1:ko)*Q; <--- eigenvectors 
    // VR=V(1:n/2,1:ko); VI=V(n/2:n,1:ko)
    // RR=VR'*(A*VR);
    // RI=VI'*(A*VI);
    // R=RR+RI;
    // eigens=eig(R); <--- eigenvalues
    // residual: resid = norm(A*VR-VR*R)+i*norm(A*VI-VI*R)
    ...
    Arnoldi::matrixMultMatrix_GPU(handle, N_rotational, k, k, V_d, 1.0, Q_Schur_d, 0.0, V1_d); //Vectors are in V1_d!!!
    //in here we have eigenvectors of the supplied mapping.
    //the rest from here requires either direct calculation of eigenvectors or their Galerkin projection.
    delete [] Q_Schur, H_Schur;

    real *residualAV_real=new real[k];
    ...
    Arnoldi::device_deallocate_all_real(1, Q_Schur_d);
    ...
    for(int i=0;i<k;i++){ 
        real ritz_val=ritz_vector[i];
        printf("\n (%.08le, %.08le), residual: %.02le+i%.02le",  (double) creal(eigenvaluesH_kk[i]), (double) cimag(eigenvaluesH_kk[i]), (double)residualAV_real[i], (double)residualAV_imag[i] );
    }
    printf("\n");
    delete [] residualAV_real, residualAV_imag;
    ...
    Arnoldi::device_deallocate_all_real(10, V_d, V1_d, vec_f_d, vec_f1_d, vec_w_d, vec_v_d, vec_c_d, vec_h_d, vec_q_d, Q_d);
